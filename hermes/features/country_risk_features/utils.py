...
def adjust_year_range(df, year_col, start_year, end_year, fill_method="null", fill_value=0):
    ...
    df_filtered = df.filter((pl.col(year_col) >= start_year) & (pl.col(year_col) <= end_year))
    ...
    if fill_method == "value":
        adjusted_df = adjusted_df.with_columns([pl.col(c).fill_null(fill_value) for c in value_cols])
    elif fill_method == "ffill":
        adjusted_df = adjusted_df.with_columns([pl.col(c).forward_fill().backward_fill() for c in value_cols])
    elif fill_method == "bfill":
        adjusted_df = adjusted_df.with_columns([pl.col(c).backward_fill().forward_fill() for c in value_cols])
